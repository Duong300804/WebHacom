...
import { updateBrand } from '../../../../api/APIs/brandApi';
import './Modal.css';

const ModalUpdate = ({ isOpen, onClose, onBrandUpdated, brand }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await updateBrand(brand.id, {
      name: formData.name,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full animate-fadeIn">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Cập nhật hãng</h3>
